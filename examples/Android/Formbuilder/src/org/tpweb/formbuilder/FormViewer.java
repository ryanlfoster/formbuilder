	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent() != null) { // send intent to show only favorite songs
			...
			String webpage = "<!doctype html><html lang=\"en\"><head><meta charset=\"UTF-8\"><title>Tjebbe's FormBuilder</title>" +
							...
							"<script>$(function() { var loadForm = \"\"; loadForm = \"\"; $('.formuliermaker').FormViewer({atabaseData: {'database':'couchdb', 'dbname':'formbuilder', 'username':'', 'password':'', 'server':'http://192.168.56.1:5984'}, database: {'database':'couchdb', 'dbname':'formbuilder', 'username':'', 'password':'', 'server':'http://192.168.56.1:5984'}, formId:'" + id + "'}); " +
							"});</script>" +
